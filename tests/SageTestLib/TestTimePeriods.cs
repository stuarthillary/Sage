...
namespace Highpoint.Sage.Scheduling
{
    ...
    public class TimePeriodTester : IDisposable
    {
        ...
        public void TestTimePeriodBasics()
        {
            ...
            Assert.True(tp.Duration.Equals(_tenMinutes), "TimePeriod Failure - initial duration on fixed start.");

            // Test modification of duration.
            tp.Duration = _fiveMinutes;
            Assert.True(tp.EndTime.Equals(_now), "TimePeriod Failure - end time on fixed start.");

            // Test modification of end time.
            tp.EndTime = _fiveMinsOn;
            Assert.True(tp.Duration.Equals(_tenMinutes), "TimePeriod Failure - duration on fixed start.");
            #endregion
            ...
            Assert.True(tp.Duration.Equals(_tenMinutes), "TimePeriod Failure - initial duration on fixed end.");

            // Test modification of duration.
            tp.Duration = _fiveMinutes;
            Assert.True(tp.StartTime.Equals(_now), "TimePeriod Failure - start time on fixed end.");

            // Test modification of start time.
            tp.StartTime = _now;
            Assert.True(tp.Duration.Equals(_fiveMinutes), "TimePeriod Failure - duration on fixed end.");
            #endregion
            ...
            Assert.True(tp.Duration.Equals(_tenMinutes), "TimePeriod Failure - initial duration on fixed duration.");

            // Test modification of start time.
            tp.StartTime = _tenMinsAgo;
            //            Assert.True(tp.EndTime.Equals(Now), "TimePeriod Failure - initial duration on fixed duration.");

            // Test modification of end time.
            tp.EndTime = _fiveMinsOn;
            //            Assert.True(tp.StartTime.Equals(FiveMinsAgo), "TimePeriod Failure - start time on fixed duration.");
            #endregion

            #region Infer Start Time
            // Test creation of a fixed start time TimePeriod.
            tp = new TimePeriod(_tenMinutes, _fiveMinsOn, TimeAdjustmentMode.InferStartTime);
            Assert.True(tp.StartTime.Equals(_fiveMinsAgo), "TimePeriod Failure - initial start time on inferred start time.");

            // Test modification of duration.
            tp.Duration = _fiveMinutes;
            Assert.True(tp.StartTime.Equals(_now), "TimePeriod Failure - changed duration on inferred start time.");

            // Test modification of end time.
            tp.EndTime = _tenMinsOn;
            //            Assert.True(tp.StartTime.Equals(FiveMinsOn), "TimePeriod Failure - changed end time on inferred start time.");
            #endregion

            #region Infer End Time
            // Test creation of a fixed end time TimePeriod.
            tp = new TimePeriod(_fiveMinsAgo, _tenMinutes, TimeAdjustmentMode.InferEndTime);
            Assert.True(tp.EndTime.Equals(_fiveMinsOn), "TimePeriod Failure - initial end time on inferred end.");

            // Test modification of start time.
            tp.StartTime = _now;
            //            Assert.True(tp.EndTime.Equals(TenMinsOn), "TimePeriod Failure - changed start time on fixed end.");

            // Test modification of duration.
            tp.Duration = _fiveMinutes;
            Assert.True(tp.EndTime.Equals(_fiveMinsOn), "TimePeriod Failure - changed duration on fixed end.");
            #endregion

            #region Infer Duration
            // Test creation of a fixed duration TimePeriod.
            tp = new TimePeriod(_fiveMinsAgo, _fiveMinsOn, TimeAdjustmentMode.InferDuration);
            Assert.True(tp.Duration.Equals(_tenMinutes), "TimePeriod Failure - initial duration on inferred duration.");

            // Test modification of start time.
            tp.StartTime = _now;
            Assert.True(tp.Duration.Equals(_fiveMinutes), "TimePeriod Failure - changed end time on fixed duration.");

            // Test modification of end time.
            tp.EndTime = _tenMinsOn;
            Assert.True(tp.Duration.Equals(_tenMinutes), "TimePeriod Failure - changed start time on fixed duration.");
            #endregion
        }
        ...
        public void TestTimePeriodEnvelope()
        {
            ...
            Assert.True(tpe.Duration.Equals(_tenMinutes), "TimePeriodEnvelope Failure a");

            tpe.AddTimePeriod(tp3);
            Assert.True(tpe.Duration.Equals(_fifteenMinutes), "TimePeriodEnvelope Failure b");

            Console.WriteLine("Removing " + tp1.ToString() + " from it.");
            tpe.RemoveTimePeriod(tp1);
            Assert.True(tpe.Duration.Equals(_tenMinutes), "TimePeriodEnvelope Failure c");
